...
for _ctr in init-cdc-zookeeper-1 init-cdc-kafka-1 init-cdc-connect-1 postgres; do
...
if ! (echo >/dev/tcp/127.0.0.1/2181) 2>/dev/null; then
    ...
    $DOCKER logs init-cdc-zookeeper-1 | tail -20
    exit 1
fi
...
if [ "$_kafka_up" -eq 0 ]; then
    ...
    $DOCKER logs init-cdc-kafka-1 | tail -30
    exit 1
fi
echo "Kafka is up."

# -- 5. Verify connector JARs are in the plugin volume -----------------------
echo "Verifying connector plugin JARs in init-cdc_kafka-plugins..."
_jar_count=$($DOCKER run --rm -v init-cdc_kafka-plugins:/plugins \
    busybox sh -c 'ls /plugins/*.jar 2>/dev/null | wc -l')
if [ "${_jar_count:-0}" -eq 0 ]; then
    echo "ERROR: No JARs found in init-cdc_kafka-plugins volume."
    echo "  The connector JARs were not loaded during postCreateCommand."
    echo "  Fix: run 'bash .devcontainer/scripts/setup-cdc.sh' then re-run this script."
    exit 1
fi
...
$DOCKER run -d \
    ...
    -v init-cdc_kafka-plugins:/kafka/connect/yugabytedb \
    quay.io/debezium/connect:${DEBEZIUM_VERSION}

$DOCKER volume create init-cdc_postgresql_data 2>/dev/null || true
$DOCKER run -d \
    --name postgres \
    --net "container:${DC_ID}" \
    --restart on-failure \
    ...
    -v init-cdc_postgresql_data:/var/lib/postgresql/data \
    postgres:14
...
[ "$_kc_ready" -eq 0 ] && {
    ...
    $DOCKER logs init-cdc-connect-1 | tail -30
    exit 1
}
