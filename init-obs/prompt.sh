...
ysqlsh -d "$DB" -X -q -c "SET yb_enable_pg_stat_statements_rpc_stats = true;" 2>/dev/null
...
pe "ysqlsh -d ${DB} -X -c \"SELECT wait_event_component, wait_event_type, wait_event, COUNT(*) AS samples, ROUND(COUNT(*)*100.0/SUM(COUNT(*)) OVER(),1) AS pct FROM yb_active_session_history WHERE sample_time >= current_timestamp - interval '5 minutes' GROUP BY wait_event_component, wait_event_type, wait_event ORDER BY samples DESC LIMIT 12;\""
...
pe "ysqlsh -d ${DB} -X -c \"SELECT lt.table_name, lt.tablet_id, a.wait_event, COUNT(*) AS samples FROM yb_active_session_history a JOIN yb_local_tablets lt ON a.wait_event_aux = SUBSTRING(lt.tablet_id,1,15) WHERE a.wait_event_component='TServer' AND a.sample_time >= current_timestamp - interval '5 minutes' GROUP BY lt.table_name, lt.tablet_id, a.wait_event ORDER BY samples DESC LIMIT 10;\""
...
pe "ysqlsh -d ${DB} -X -c \"SELECT s.host, s.zone, a.wait_event_component, a.wait_event_type, COUNT(*) AS samples, ROUND(COUNT(*)*100.0/SUM(COUNT(*)) OVER(PARTITION BY s.host),1) AS pct_of_node FROM yb_active_session_history a JOIN yb_servers() s ON a.top_level_node_id=s.uuid::uuid WHERE a.sample_time >= current_timestamp - interval '5 minutes' GROUP BY s.host,s.zone,a.wait_event_component,a.wait_event_type ORDER BY samples DESC;\""
...
pe "ysqlsh -d ${DB} -X -c \"SELECT a.pid, sa.state, a.wait_event, COUNT(*) AS ash_samples, ROUND(sa.allocated_mem_bytes/1024.0/1024,1) AS mem_mb FROM yb_active_session_history a LEFT JOIN pg_stat_activity sa ON a.pid=sa.pid WHERE a.sample_time >= current_timestamp - interval '5 minutes' AND a.wait_event_component='YSQL' GROUP BY a.pid,sa.state,a.wait_event,sa.allocated_mem_bytes ORDER BY ash_samples DESC LIMIT 8;\""

pe "ysqlsh -d ${DB} -X -c \"SELECT pid, state, wait_event_type, wait_event, LEFT(query,60) AS query, now()-query_start AS query_age FROM pg_stat_activity WHERE state!='idle' AND pid!=pg_backend_pid() ORDER BY query_age DESC NULLS LAST;\""
...
pe "ysqlsh -d ${DB} -X -c \"SELECT pid, granted, mode, ybdetails->>'transactionid' AS txn_id, ybdetails->>'tablet_id' AS tablet_id, ybdetails->>'blocked_by' AS blocked_by, waitstart FROM pg_locks WHERE locktype='keyrange' ORDER BY granted;\""

BLOCKER_TXN=$(ysqlsh -d "$DB" -X -t -q -c "
...
if [ -n "$BLOCKER_TXN" ]; then
    ...
    pe "ysqlsh -d ${DB} -X -c \"SELECT yb_cancel_transaction('${BLOCKER_TXN}');\""
...
pe "ysqlsh -d ${DB} -X -c \"EXPLAIN (ANALYZE, DIST) SELECT o.order_id, o.amount FROM orders o WHERE o.customer_id = 42 ORDER BY o.created_at DESC LIMIT 20;\""

p "Storage Table Rows Scanned >> 20. Seq Scan on orders confirmed."
p "Fix: covering index."

pe "ysqlsh -d ${DB} -X -c \"CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_customer_id ON orders (customer_id) INCLUDE (created_at, amount, status, product_id);\""
...
pe "ysqlsh -d ${DB} -X -c \"SELECT wait_event, COUNT(*) AS samples FROM yb_active_session_history WHERE sample_time >= current_timestamp - interval '3 minutes' AND wait_event_component='YSQL' GROUP BY wait_event ORDER BY samples DESC LIMIT 10;\""
...
if [ -n "$QUERY_ID" ]; then
    pe "ysqlsh -d ${DB} -X -c \"SELECT yb_query_diagnostics(query_id => ${QUERY_ID}, diagnostics_interval_sec => 30, explain_sample_rate => 25, explain_analyze => true, explain_dist => true, bind_var_query_min_duration_ms => 5);\""

    sleep 5

    pe "ysqlsh -d ${DB} -X -c \"SELECT query_id, state, folder_path, status FROM yb_query_diagnostics_status ORDER BY start_time DESC LIMIT 3;\""
