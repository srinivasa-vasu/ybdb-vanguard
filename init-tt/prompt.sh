...
pe "ysqlsh -c \"SELECT COUNT(*) AS total_accounts, ROUND(SUM(balance), 2) AS total_funds, ROUND(AVG(balance), 2) AS avg_balance FROM payments;\""
...
pe "ysqlsh -c \"UPDATE payments SET balance = balance * 0.10 WHERE id % 20 = 0;\""

pe "ysqlsh -c \"DELETE FROM payments WHERE balance > 9500 AND id <= 100;\""

pe "ysqlsh -c \"SELECT COUNT(*) AS remaining_accounts, ROUND(SUM(balance), 2) AS remaining_funds FROM payments;\""
...
pe "ysqlsh -c \"SET yb_read_time TO ${READ_TS_US}; SELECT COUNT(*) AS accounts_before, ROUND(SUM(balance), 2) AS funds_before FROM payments; SET yb_read_time TO 0;\""
...
pe "ysqlsh -c \"SET yb_read_time TO ${READ_TS_US}; SELECT id, customer, balance AS balance_before FROM payments WHERE id % 20 = 0 ORDER BY id LIMIT 10; SET yb_read_time TO 0;\""

p "Those same accounts now:"

pe "ysqlsh -c \"SELECT id, customer, balance AS balance_after FROM payments WHERE id % 20 = 0 ORDER BY id LIMIT 10;\""
...
pe "ysqlsh -c \"SET yb_read_time TO ${READ_TS_US}; SELECT id, customer, balance AS last_known_balance FROM payments WHERE balance > 9500 AND id <= 100 ORDER BY balance DESC; SET yb_read_time TO 0;\""

p "Those rows in the CURRENT database:"

pe "ysqlsh -c \"SELECT COUNT(*) AS count_now FROM payments WHERE balance > 9500 AND id <= 100;\""
...
p "Accounts before  : 1,000   |   Accounts now: $(ysqlsh -t -c "SELECT COUNT(*) FROM payments;" | xargs)"
p ""
